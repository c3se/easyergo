...
async def check_known_kws(ls,  params=types.DocumentDiagnosticParams):
    ...
    for _, m in query_dep_spec.matches(tree.root_node):
        ...
        if 'dep.toolchain' in m and eval(m['dep.toolchain'].text)==b'SYSTEM':
            tcs = [{'name': 'system', 'version': 'system'}]
            # give up on more complex toolchain spec here
        else:
            tcs = default_tcs
        ...
        if matches:
            ...
            if name_exists:
                ...
                diagnostics.append(make_diagnostic(name_node, 'Name not recognized'))
